...
- **Sources**: YouTube Music (InnerTube search, related tracks, artist pages), JioSaavn
  (320 kbps AAC), Audius (320 kbps MP3). YouTube audio is Opus/AAC at roughly 128–160 kbps;
  where a JioSaavn match for the same song exists it is used instead.
...
- The streaming-quality preference is stored but not yet applied to stream selection, so its
  picker is hidden until Phase 1 wires it.
...
| 1 | `play` / `plus` product flavors, `MusicSource` abstraction, local library | Planned |
