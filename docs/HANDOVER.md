...
| **E. Privacy** | Chat, dedications and lyric moments **encrypted client-side** with a key derived from the room code (HKDF). The relay stores ciphertext it cannot read. Playback state stays plaintext — the relay needs the timestamps and they are not sensitive. |
