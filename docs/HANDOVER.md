...
| Tests | **345** on `play`, **63** in `relay/`, plus **22** end-to-end checks against a running relay. 0 failures. The relay's end-to-end checks run in CI now, because the relay is an ordinary process CI can start |
...
| **G. Chat that persists** | **Done** — `together_messages` (Room v5, additive migration tested against a real v4 database). A message is written down before it is sent, so a failed send leaves an outbox row holding the exact bytes; reconnecting resends the identical frame and the relay's echo collides with that row rather than doubling it. The id is a digest of the ciphertext, which is what makes a rejoin's fifty-message replay idempotent. Readable in the Together tab without starting a session. |
| **H. The call** | **Built, never run.** `WebRtcEngine` (one peer connection, mic, camera), `CallController` (a pure state machine, 28 tests), `CallSession` (the glue, on the Together session's own thread), `CallService` (so a call survives the screen going off), `CallOverlay`. Signalling rides the encrypted chat channel, so the relay never learns a call is happening. TURN comes from the relay and is verifiable with `npm run check:turn`. **What no test here can tell you is whether a peer connection forms between a phone in the UAE and one in India**, which is the only question that matters about it. |
